...
if __name__ == '__main__':
    ...
    par_apply_masking = [True, False]
    par_sl_w = [1, 2.5]
    par_sl_smoothing = [None, 0.1]
    par_rot_w = [1, 2.5, 5]
    par_rot_prob = [0.5, 1.0]

    par_combinations = list(product(par_apply_masking, par_sl_w, par_sl_smoothing, par_rot_w, par_rot_prob))

    print(len(par_combinations))

    configs = []
    for combination in par_combinations:
        apply_masking, sl_w, sl_smoothing, rot_w, rot_prob = combination
        configs.append({'apply_masking': apply_masking,
                        'sl_w': sl_w, 
